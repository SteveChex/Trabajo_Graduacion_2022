...
timestamp_h = [50, 100, 150, 200, 220, 260, 280]
heightChanges = [0.2, 0.5, 0.7, 0.2, 0.9, 0.7, 0.5]
...
def initParams(cf):
    cf_max_vel_xy = 0.4
    cf_max_vel_z = 0.5
    cf.param.set_value('posCtlPid.xyVelMax', cf_max_vel_xy)
        ...
        try:
            while not keyboard.is_pressed('space'):  # if key 'space' is pressed
                ...
                while not keyboard.is_pressed('space'):
                    cf.commander.send_zdistance_setpoint(0,0,0,0.2)
                    #cf.commander.send_hover_setpoint(0,0,0.5,0)                        
                    #cf.commander.send_position_setpoint(0, 0.5, 0.5, 0)
                    #if it > 700:
                    #    #cf.param.set_value("flightmode.poshold",1)
                    #    pass
                    it+=1
                    if it > timestamp_h[iterVar]:
                        iterVar += 1
