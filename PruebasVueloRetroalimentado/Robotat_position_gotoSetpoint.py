...
def sliceTraj(initPoint, goalPoint, minSlice):
    ...
    for item in range(0,len(goalPoint)):
        ...
        if numSlices[item] == 0:
            zeroDetection[item] = 1
        else:
            zeroDetection[item] = 0
    print(numSlices)
    if not zeroDetection[0] and not zeroDetection[1]:
        print("Not zero on coord detected")
        numSlices = abs(max(numSlices))
        ...
        print("zero detected on (x, y): ({:d}, {:d})", zeroDetection[0], zeroDetection[1])
        numSlices = abs(max(numSlices))
        for item in range(0, numSlices + 1):
            if zeroDetection[0]:
                distx = initPoint[0]
                disty = (goalPoint[1] - initPoint[1])*item/numSlices + initPoint[1]
            elif zeroDetection[1]:
                distx = (goalPoint[0] - initPoint[0])*item/numSlices + initPoint[0]
                disty = initPoint[1]
            trajx.append(round(distx,4))
            trajy.append(round(disty,4))
