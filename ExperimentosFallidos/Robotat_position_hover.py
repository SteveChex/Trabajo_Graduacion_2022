...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        try:
            ...
            endFlags = [0, 0]
            testOnly = False
            #startPoint = [Robotat.bodypos[0]+0.2, Robotat.bodypos[2]-0.2]
            #cf.param.set_value("flightmode.poshold",0)
            cf.commander.send_setpoint(0,0,0,0)
            while not keyboard.is_pressed('space'):
                pos = Robotat.bodypos
                quat = Robotat.bodyquat
                inBorders = check_inPoint([pos[0], pos[2]], startpoint, 0.05)
                eul = euler_from_quaternion(quat[0], quat[1], quat[2], quat[3])
                eulL = list(eul)
                eulL[1] = eulL[1] + 70
                eul = tuple(eulL)
                ...
                if endFlags[1] == 1 and endFlags[0] == 1:
                    ...
                    break
                print("\nPosition XZ  \t{:.4f}\t{:.4f}\
                        \nR P h Parameters           \t{:.1f}\t{:.1f}\t{:.1f}\t\
                        \narrived \t{:d}\
                        \nendFlags\t{:d}\t{:d}\t"
                        .format(pos[0], pos[2],
                                roll, pitch, pos[1],\
                                    arrived,
                                    endFlags[0], endFlags[1]),end = "")
